...
abstract class Twig_Template implements Twig_TemplateInterface
{
    ...
    protected function displayWithErrorHandling(array $context, array $blocks = array())
    {
        try {
            ...
            throw new Twig_Error_Runtime(sprintf('An exception has been thrown during the rendering of a template ("%s").', $e->getMessage()) . $e, -1, null, $e);
        }
    }
    ...
    protected function getAttribute($object, $item, array $arguments = array(), $type = Twig_TemplateInterface::ANY_CALL, $isDefinedTest = false, $ignoreStrictCheck = false)
    {
        ...
        if (Twig_TemplateInterface::METHOD_CALL !== $type) {
            if (isset($object->$item)
                || (($object instanceof CComponent) && $object->hasProperty($item))
                || array_key_exists($item, $object)) {
                if ($isDefinedTest) {
                    return true;
                }
